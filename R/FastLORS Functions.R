...
rankHC <- function(Bhat_standardized){
  ...
  for (j in 1:nrow(Bhat_standardized)){
    if(length(which(Bhat_standardized[j,] == 0)) != ncol(Bhat_standardized)){  #### for very sparse matrices, some rows may be all 0.  Don't want to calculate HC for these
      ...
      q <- length(Bhat_standardized[j,])
      HC = max(sqrt(q)*(S(t_vec, Bhat_standardized[j,])/q - survival(t_vec)) / sqrt(survival(t_vec) * (1 - survival(t_vec))))
      HC_vec <- c(HC_vec, HC)
    }
...
#' @param tune_method chooses whether LORS or Fast-LORS should be used in parameter tuning.
#' @param seed the random seed for spliting data into training and validation for parameter tuning
#' @param maxiter maximum number of iterations
#' @param eps constant used when checking the convergence.  Ensures no division by 0.
#' @param tol tolerance level for convergence
#' @param cross_valid chooses whether two-fold cross-validation should be used in parameter tuning
#' @export
#' @examples
#'
#' ##Example
...
Run_LORS <- function(Y, X, method = "Fast-LORS", screening = "HC-Screening", tune_method = "Fast-LORS", seed = 123,  maxiter = 10000, eps = 2.2204e-16, tol = 1e-4, cross_valid = TRUE){
  ...
  lambda <- mean(params1[["lambda"]], params2[["lambda"]])
  rho <- mean(params1[["rho"]], params2[["rho"]])
  end_param <- proc.time()
